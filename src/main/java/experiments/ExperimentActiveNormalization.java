...
public class ExperimentActiveNormalization {
    ...
    private List<ArtificialOracle> getOracles(int nbTransactions) {
        ...
        oracles.add(new OWAOracle(0.01, measureNames.length));
        oracles.add(new ChiSquaredOracle(nbTransactions));
    ...
    private List<IterativeRankingLearn> getLearningAlgorithms(ArtificialOracle oracle, Dataset dataset) {
        ...
        normMethods.add(NormalizationMethod.EMPIRICAL_CDF);
        normMethods.add(NormalizationMethod.Z_NORMALIZATION);

        for (NormalizationMethod norm : normMethods) {
            String normMethod = norm.name();

            KappalabIterative topTwoRules = new KappalabIterative(nbLearningIterations,
                    new TopTwoRules(oracle, dataset, measureNames, 0.0d, 10_000, norm), new LinearScoreFunction(),
                    measureNames.length);
            topTwoRules.setName("TopTwoRules-" + normMethod);
            ...
            algorithms.add(uncertaintySamplingTh);
        }
    ...
    public void runParallel() throws Exception {
        for (String datasetName : datasetNames) {
            try {
                ...
                IntStream.range(0, trainDatasets.size()).forEach(foldIdx -> {
    ...
    public static void main(String[] args) throws Exception {
        new ExperimentActiveNormalization().runParallel();
    }
}
